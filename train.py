...
def build_custom_memories(episodes: List[EpisodeBatch], knobs: ResearchConfig) -> Dict[str, np.ndarray]:
    vision_keys = []
    multimodal_keys = []
    instant_actions = []
    chunk_keys = []
...
def select_action(
    ...
    for similarity, chunk in zip(similarities, chunks):
        ...
        if knobs.use_chunk_decoder:
            ...
            with torch.no_grad():
                decoder_score = float(predictive_bundle.chunk_decoder(decoder_state, decoder_input).item())
...
def main() -> None:
    ...
    if env is not None:
        raise SystemExit("Root train.py is intended for the offline LIBERO real sample only.")
